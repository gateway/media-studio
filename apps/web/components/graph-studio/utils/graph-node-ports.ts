...
export function visibleGraphInputPorts(definition: GraphNodeDefinition, fields: Record<string, unknown>): GraphNodePort[] {
  ...
  return definition.ports.inputs.filter((port) => !port.advanced);
}
...
export function visibleGraphOutputPorts(definition: GraphNodeDefinition, fields: Record<string, unknown>): GraphNodePort[] {
  ...
  if (definition.type === "video.transform") {
    return definition.ports.outputs.filter((port) => port.id === "video");
  }
  if (definition.type === "video.extract") {
    ...
    return definition.ports.outputs.filter((port) => port.id === outputId);
  }
  if (definition.type === "audio.transform") {
    ...
    return definition.ports.outputs.filter((port) => (operation === "extract_metadata" ? port.id === "metadata" : port.id === "audio"));
  }
  return definition.ports.outputs.filter((port) => !port.advanced);
}
