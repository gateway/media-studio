...
import { getControlPlaneSnapshot, getMediaPreset } from "@/lib/control-api";
...
export default async function EditMediaPresetPage({
  ...
  const [snapshot, presetResult] = await Promise.all([
    getControlPlaneSnapshot(),
    getMediaPreset(resolvedParams.presetId),
  ]);
  const preset = presetResult.data.preset;
  if (!preset) {
    notFound();
  }
