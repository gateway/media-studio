...
export async function GET(
  ...
  const batchSnapshot = await getControlApiJson<Record<string, unknown>>(
    `/media/batches/${batchId}`,
    "read",
  );
  if (!batchSnapshot.ok || !batchSnapshot.data) {
    return controlErrorResponse(
      batchSnapshot.error,
      "Unable to read the current media batch state.",
      502,
    );
  }
  ...
  if (!activeJobs.length) {
    const jobs = (batchSnapshot.data.jobs as Record<string, unknown>[]).map(
      mapJobRecord,
    );
  ...
  await Promise.all(
    activeJobs.map((job) =>
      postControlApiJson<Record<string, unknown>>(
        `/media/jobs/${String(job.job_id ?? "")}/poll`,
        { wait: false },
        "admin",
      ).catch(() => null),
    ),
...
export async function POST(
  ...
  if (!result.ok || !result.data) {
    return controlErrorResponse(
      result.error,
      "Unable to cancel queued media jobs for this batch.",
      502,
    );
  }

  const jobsResult = await getControlApiJson<{
    items?: Record<string, unknown>[];
  }>("/media/jobs?limit=200", "read").catch(() => null);
  const jobs = Array.isArray(jobsResult?.data?.items)
    ? jobsResult?.data?.items.map(mapJobRecord)
    : [];
