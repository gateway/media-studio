...
ASSISTANT_PACKAGE_CAP = runpy.run_path(str(GUARDRAIL_SCRIPT))["ASSISTANT_PACKAGE_GUARDRAIL"].max_lines
...
def test_assistant_package_growth_fails_with_review_guidance(tmp_path: Path) -> None:
    ...
    assert f"apps/api/app/assistant: {ASSISTANT_PACKAGE_CAP + 1} lines exceeds max {ASSISTANT_PACKAGE_CAP}" in result.stderr
...
def test_assistant_package_ignores_generated_and_non_python_artifacts(tmp_path: Path) -> None:
    ...
    assert f"     1 {ASSISTANT_PACKAGE_CAP:>6}  apps/api/app/assistant  [OK]" in result.stdout
