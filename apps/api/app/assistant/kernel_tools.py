...
KERNEL_TOOLS: Dict[str, KernelToolDefinition] = {
    ...
    "inspect_generation": KernelToolDefinition(name="inspect_generation", description="Inspect the exact current provider-bound prompt without generation, or session-owned authored/prepared/submitted/rejected run evidence in chunks. With evidence=source and an exact run_id/node_id, recover the saved upstream graph and retained inputs in source_context chunks; follow next_offset. Use for truncation, failed recipe diagnosis and configured model readiness. Dynamic recipe outputs report pending, never certified. Optional read-only fresh balance is not proof of model funding. Never runs, uploads, retries, or grants approval.", arguments_model=InspectGenerationArguments, allowed_capabilities=frozenset({"general", "graph_builder", "recipe_builder", "preset_builder", "run_debugger"}), handler=inspect_generation),
