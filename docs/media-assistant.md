...
The restart-specific timeout has a bounded fix. A provider thread resumed in a fresh API process has no in-memory prior-usage counters, so the first-turn compaction check previously read zero and skipped compaction. Disk-resumed threads now compact once before the first eligible user turn. Browser session `asst_df929376ff42` survived an API reload, recorded `thread_resumed` then `thread_compacted`, and returned the grounded preset prerequisite with `next_action=none` in 40.7 seconds instead of timing out at 90 seconds. This closes the restart timeout but does not satisfy the ordinary 30-second p95 boundary.

Remaining release blockers:

- Ordinary p95 latency, token volume, provider-step distribution, and the incomplete D1–V3 human browser matrix still exceed the release boundary. The G1–G3/P5/R3–R4 fixture mismatches and the post-restart recall timeout are resolved; reference analysis remained within its separate 60-second p95 ceiling in all three Ticket 09 samples.
