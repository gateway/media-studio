...
`scripts/check_file_size_guardrails.py` caps the recursively counted Python source under `apps/api/app/assistant/` at 11,395 lines, the exact total on the reviewed Ticket-9 responsiveness candidate. The 96-line increase from the post-Ticket-7 total adds one bounded, deterministic story-to-image graph compiler through the existing proposal, validation, pricing, and layout seam. It replaces provider-authored 31-operation graph payloads; there was no equivalent existing template to reuse or redundant production code to remove without weakening the requested story-graph capability. The count includes every `*.py` source file, ignores `__pycache__` and non-Python artifacts, and runs in both `npm run quality:assistant-ci` and the release quality gates. Individual-file caps remain in force, so the package cap cannot be bypassed by splitting a large module.
...
Ticket 09 has a conclusive **NO-GO**, but its full signoff checklist is not complete: the exact mechanical matrix and the five-turn story browser journey ran, while the remaining approved D1–V3 conversations were not all human-scored. The Assistant therefore stays opt-in. Do not describe Ticket 09 as complete until that browser matrix is run against a later blocker-fixed candidate; no additional paid Seedance proof is required because the user explicitly owns that deferred check.

Remaining release blockers:

- Ordinary p95 latency, token volume, provider-step distribution, the reproducible G1–G3/P5/R3–R4 capability contracts, and the post-restart recall timeout exceed the release boundary. Reference analysis remained within its separate 60-second p95 ceiling in all three Ticket 09 samples.
