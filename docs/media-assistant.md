...
`scripts/check_file_size_guardrails.py` caps the recursively counted Python source under `apps/api/app/assistant/` at 11,527 lines. Ticket 11's reviewed story-tail candidate established 11,453 lines; the long-turn follow-up adds 74 lines by reusing the existing single-flight tracker for an ephemeral progress response, publishing already-typed safe activity labels, coordinating the established kernel time budget, and retaining single-flight ownership through final turn persistence. That is smaller and safer than introducing persisted progress, a worker, streaming transport, another planner, or broad retry behavior. The earlier increases remain the bounded deterministic story compiler, typed artifact-completion/presentation contracts, and measured provider-generation refresh. The count includes every `*.py` source file, ignores `__pycache__` and non-Python artifacts, and runs in both `npm run quality:assistant-ci` and the release quality gates. Individual-file caps remain in force, so the package cap cannot be bypassed by splitting a large module.

The 2026-08-30 PR-readiness review kept that cap unchanged. It moved node measurement, rectangle geometry, group bounds, and dynamic Media Preset sizing from the Assistant's layout policy into the graph-owned layout module, reducing the Assistant package from 11,622 to 11,474 lines. This is an ownership correction rather than a file split: Graph Studio owns node geometry, while the Assistant module retains only its production-arrangement policy.
...
The restart path now restores the last persisted provider-usage measurement before evaluating the existing compaction threshold; below-threshold threads are not compacted. The exact five-turn story session `asst_0c2d94554719` then reproduced its original post-restart recall timeout at 92.3 seconds. The persisted message trail showed an orphaned user turn from the prior 502, so a subsequent request now detaches that stalled provider thread and hydrates a fresh one from the existing typed story state and recent conversation. The same restored canvas and exact read-only recall then returned the correct Shot 4 revision in 10.5 seconds (9.7 seconds provider latency), with lifecycle `thread_started`, zero tool calls, `next_action=none`, and no graph mutation or run. This closes the restart-recall failure without forcing compaction or hiding the measured provider time.

Remaining release blockers:

- Provider latency, token volume, and provider-step distribution remain monitoring evidence. The D1–V3 human browser matrix is complete, and the G1–G3/P5/R3–R4 fixture mismatches plus exact post-restart five-turn story recall are resolved. The release blockers are now limited to reproducible application/tool stalls, false or missing progress, cancellation failures, HTTP 5xx responses, step/wall-clock exhaustion, and the cost ceiling defined above.
...
Ticket 11 is **incomplete-NO-GO**. Its first six-shot draft timed out at the 90-second boundary before a follow-up produced the required distinct prompts. The cleanup turn then exposed two general geometry defects: stale production sections stacked vertically, and dynamic Prompt Recipe layout counted every recipe field rather than the selected recipe's visible contract. Candidate `8e98a95894805be4dcb5a4d16726fe66c42e45fd` fixes those owners test-first, keeps large disconnected layouts bounded to three columns, excludes generic title words from continuity-note association, and expands measured group frames only for rendering so saved workflow geometry is not silently rewritten. The visible Media Assistant cleanup and reload restored the same 13 nodes, 8 typed edges, 3 groups, comments, and selected recipe identities in a compact Character Sheet → Six-Shot Storyboard → Shot 5 Video layout. A subsequent `Run it` request stopped at explicit confirmation for the 530-credit / $2.65 estimate; no provider job ran and credits remained `2,838.6`. Three fresh-workflow negative checks also passed: conversation-only guidance kept a zero-node graph, declining two exact character-sheet recipes moved to the direct-model path without repetition, and an ambiguous storyboard request offered two exact saved recipes and asked the user to choose instead of guessing. The exact implementation candidate passed Assistant CI and the full release gate with 841 backend tests, 738 web tests, lint, typecheck, production build, Studio smoke, and schema version 54 with no pending migrations. The J8 grounded-summary request timed out twice at the product boundary, and the original J5 timeout remains a strict rubric zero, so this is not a complete signoff.

Ticket 11 is now a conclusive **complete-NO-GO** on runtime candidate `5fa85327a27fa3efad3442873294e8a6c394d70c`. The remaining locally owned defects are closed: collection normalization reuses one catalog snapshot, starter templates wait for definitions, mixed recipe-backed production graphs retain their typed graph-run confirmation, and stored shot lists render every validated prompt instead of a vague success summary. The full 25-node creative-production graph restored with 17 typed edges and three padded left-to-right groups; its grounded read-only summary and 556-credit / $2.78 `Review and run` boundary passed with zero jobs and zero spend. A fresh final-browser session reproduced one remaining release blocker: the first exact six-shot request timed out at 89 seconds, while the existing stalled-thread recovery completed in 67.8 seconds and visibly returned all six stored prompts. Assistant CI passed, and the full release gate passed 844 backend tests, 739 web tests, lint, typecheck, build, smoke, and clean schema version 54. The Assistant remains opt-in; no timeout increase, reasoning-policy exception, keyword route, or broad retry was added.

The first-attempt six-shot tail is resolved in the subsequent worktree candidate based on `9927fd3b14948ef31541bf9b852435cad6894a75`. Reused Codex Local threads now advance through the existing provider-generation seam before a typed story turn when the last persisted provider prompt reaches 45,000 tokens. This deliberately narrow rule is based on failures at 47,064 and 48,250 tokens and comparable successes at or below 40,689; it does not affect general conversations, other providers, models, reasoning policy, wall-clock budgets, or retry behavior. In visible session `asst_2d4a163f0dcd`, the precursor reached 45,547 tokens and the exact six-shot request completed on its first attempt in 72.0 seconds with a fresh 31,680-token prompt, one provider process, and all six typed prompts visible. J1–J7 and an ordinary browser reload passed continuously with no graph run or spend. A full launcher restart then proved the changed lifecycle against persisted 25-node session `asst_5bcfe5d941e4`: grounded recall completed in 33.0 seconds on a new provider generation, preserved the exact Shot 3 audio detail, and `Run it` stopped at the 556-credit / $2.78 confirmation boundary. The fifth unsaved browser tab from the new walk was not retained by that full launcher restart; its Assistant transcript remained addressable by the supported session URL, but its tab-local graph was not. That browser-tab retention limitation is separate from provider-thread lifecycle and is not expanded into this fix. The Assistant remains opt-in. Under the owner-approved release boundary, the measured 72.0-second model/provider turn is monitoring evidence rather than a blocker because truthful progress stayed visible and no application/tool stall or budget exhaustion occurred.

The long-turn follow-up changes the product boundary from a 90-second kernel / 130-second browser pair to a 180-second kernel / 220-second browser pair while retaining the six-tool-step limit and explicit Stop action. Active chat, planning, and preset/recipe drafting turns expose only ephemeral server-owned progress: elapsed time, a neutral thinking heartbeat, and completed typed tool-activity labels. The browser polls that state every two seconds and explains after 120 seconds that the Assistant is still working and can be stopped. Stop is shown only for those single-flight kernel turns; it retains ownership through final persistence, clears the active/progress registries before reporting success, and keeps a retryable stopping state if cleanup exceeds five seconds. Direct artifact saves retain their established non-cancellable persistence boundary and do not claim tracked progress. Provider reasoning text is never returned, progress is removed when the turn ends, and no database schema, saved-session contract, worker, streaming transport, dependency, provider model, or reasoning policy changes.
