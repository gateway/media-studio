...
usage() {
  cat <<'EOF'
Usage: ./scripts/start_web.sh [--host HOST] [--port PORT] [--api-host HOST] [--api-port PORT] [--control-api-base-url URL]
EOF
}
...
cd "$MEDIA_ROOT"
exec npm --workspace apps/web run start -- --hostname "$WEB_HOST" --port "$WEB_PORT"
